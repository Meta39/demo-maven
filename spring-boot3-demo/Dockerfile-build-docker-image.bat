...
start /B /WAIT cmd /c "mvn -T %THREAD% -D maven.test.skip=true -P %PROFILES% clean package -q -o -f pom.xml"
...
start /B /WAIT cmd /c "mvn -T %THREAD% dockerfile:build -q -o -f pom.xml"
if %ERRORLEVEL% neq 0 (
...
start /B /WAIT cmd /c "mvn dockerfile:push -q -o -f pom.xml"
if %ERRORLEVEL% neq 0 (
