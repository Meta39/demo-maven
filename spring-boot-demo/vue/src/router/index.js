    ...
    if (!currentRouteNames.includes('Manage')) {
      ...
      menus.forEach(item => {
        if (item.path) {  // 当且仅当path不为空的时候才去设置路由
          let itemMenu = { path: item.path.replace("/", ""), name: item.menuName, component: () => import('../views/' + item.name + '.vue')}
          manageRoute.children.push(itemMenu)
        } else if(item.childAuthorize.length) {
          item.childAuthorize.forEach(item => {
            if (item.path) {
              let itemMenu = { path: item.path.replace("/", ""), name: item.menuName, component: () => import('../views/' + item.name + '.vue')}
              manageRoute.children.push(itemMenu)
            }
...
router.beforeEach((to, from, next) => {
  ...
  if (!to.matched.length) {
    localStorage.getItem("menus") ?next("/404"):next("/login")
  }
  // 其他的情况都放行
  next()
